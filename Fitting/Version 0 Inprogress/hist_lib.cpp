...
// void hist_lib::get_range(double ret_range[]){
//   ret_range[0] = range[0];
//   ret_range[1] = range[1];
// }

// int hist_lib::get_nbins(){
//   return int((range[1]-range[0])/binsize);
// }

// int hist_lib::get_xysize(){
//   return xysize;
// }
...
int ** hist_lib::compute_hist(double c1[],double c2[],int cnum){
  static int ** retvals;

  //initialize two dimensional integer array (Dynamic)
  retvals = new int*[xysize];
  for (int i=0;i<xysize;i++){
    retvals[i] = new int[xysize];
    for (int j=0;j<xysize;j++){
      retvals[i][j] = 0;
    }
  }
