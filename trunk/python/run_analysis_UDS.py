...
mfile="UDS_TESTd_model.fits"
templatefile="../templates/default_templates.fits" # make sure to follow path
obsfile="../obs/UDS_complete_B.fits"
outfile="UDS_TESTd_output.fits"

## Info on this specific run 
## ------------------------
## run #: 2
## specs: UDS, 1.1 mm and 24 um complete sources 
## notes: highly limited run.. all parameters fixed
##        at G13 values except P,Q, P2,Q2 
...
mod.filters[0].limit=1.75               # mJy lower flux limit 
mod.filters[0].err=1.7915               # characteristic flux measurement error

mod.filters[1].setID("Spitzer_MIPS_24um")
mod.filters[1].limit=0.043
mod.filters[1].err=0.134

mod.filters[2].setID("Spitzer_IRAC_4") # according to IRAC instrument handbook
mod.filters[2].limit=0.0047                 # http://irsa.ipac.caltech.edu/data/SPITZER/...
mod.filters[2].err=0.005
...
mod.params['cexp'].pmin=0.0 # fixed 
